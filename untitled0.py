...
"""
=============================================================================
BOSCH | PCB Lesson Learn Quality Studio (Wide-Band Centered Blue Matrix Edition)
- Solves Right-Side Clamping: Breaks free from narrow side cell to full width
- Zero Image Clipping: Generates spacious parallel blue boxes (No boundary cut)
- True Symmetrical Center Alignment: 100% centered with room for details
...
def build_spacious_centered_blue_matrix(table, row_idx, cell_idx, img_bytes_list):
    """
    【彻底破局：宽幅居中独立并列蓝框渲染器】
    - 针对多张图片，彻底释放右侧狭窄单元格的枷锁；
    - 将整个单元格设为全宽水平居中容器 (w:jc=center)；
    - 根据图片数量 N，创建宽敞并列的独立深蓝相框 (单张图宽度达 1.4~1.5 英寸)；
    - 两侧留足安全呼吸空间，绝不被右侧边界裁切！
    """
    ...
    # 2. 多张图片：构建宽幅居中独立并排蓝底相框
    # 抹掉外层大蓝框，将其作为全宽容器
    set_cell_background_color(target_cell, "FFFFFF")
    
    # 建立独立网格子表格，行0为深蓝标题，行1为图片内容
    ...
    # 科学计算可用宽度：为防止最右侧被裁切，留出 0.25 英寸右侧安全区
    safe_total_width = 3.0
    col_w = safe_total_width / count
    # 图像宽度设为列宽的 85%，确保右侧边缘留白，绝不撞墙裁切
    ...
    for i in range(count):
        ...
        set_cell_background_color(c_h, "005691") # 独立深蓝
        ...
        # B. 底部独立图片容器 (浅灰底色 + 细实线边框 + 水平垂直绝对几何居中)
